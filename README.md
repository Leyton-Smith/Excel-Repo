...
## Overview
This project was completed as part of a Data Technician Bootcamp and demonstrates practical Excel skills used for data cleaning, transformation, analysis, and visualisation. The project involved working with retail sales, customer demographics, product sales, and bike sales datasets to extract insights and present findings using industry-standard Excel techniques.

## Objectives
- Analyse retail and sales datasets.
- Perform data aggregation and calculations using Excel formulas.
- Create PivotTables to summarise and explore datasets.
- Apply data visualisation techniques to communicate insights.
- Use conditional formatting to highlight trends and patterns.
- Develop business-focused reporting and analytical thinking.
...
- `SUMIF()` for conditional totals and demographic analysis.
- `AVERAGE()` for calculating mean values.
- `AVERAGEIF()` for analysing customer spending and age by category.
- `DATE()`, `MONTH()`, and `YEAR()` for working with date-based data.
- `UNIQUE()` for extracting distinct values.
- `VLOOKUP()` for retrieving related information from tables.
...
- Support faster identification of trends and outliers.
